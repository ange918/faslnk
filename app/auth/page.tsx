...
export default function AuthScreen() {
  ...
  const handleFinalSubmit = (e: React.FormEvent) => {
    ...
    login({
      ...
      prenom: formData.prenom || "Ange",
      email: formData.email || "demo@fashlink.com",
      role: "Styliste Senior"
    });
  };

  return (
    <div className="min-h-screen bg-black text-white p-8 flex flex-col pt-24 max-w-md mx-auto animate-in fade-in duration-700">
      ...
      <AnimatePresence mode="wait">
        {isLogin ? (
          <motion.form
            ...
            className="space-y-5"
          >
            <Input label="Email" type="email" placeholder="votre@email.com" required value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} />
          ...
          >
            {step === 1 ? (
              <form onSubmit={handleNextStep} className="space-y-5">
                <Input label="Nom" placeholder="Nom" required value={formData.nom} onChange={(e) => setFormData({...formData, nom: e.target.value})} />
                <Input label="Prénom" placeholder="Prénom" required value={formData.prenom} onChange={(e) => setFormData({...formData, prenom: e.target.value})} />
                <Input label="Email" type="email" placeholder="votre@email.com" required value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} />
                <Input label="Mot de passe" type="password" placeholder="••••••••" required />
              ...
              <form onSubmit={handleFinalSubmit} className="space-y-5">
                ...
                <Input label="Date de naissance" type="date" required value={formData.dob} onChange={(e) => setFormData({...formData, dob: e.target.value})} />
      ...
      <div className="mt-auto pt-10 space-y-6">
        ...
        <button type="button" onClick={handleFinalSubmit} className="w-full flex items-center justify-center gap-3 px-4 py-4 border border-white/10 rounded-2xl hover:bg-white/5 transition-all text-sm font-medium">
          <GoogleIcon />
